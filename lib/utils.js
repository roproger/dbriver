...
exports.parseExpression = (expression) => {
  ...
  if (Array.isArray(expression))
    return `(${value.map((v) => exports.parseExpression(v)).join(",")})`
...
exports.parseValue = (value) => {
  ...
  if (Array.isArray(value))
    return `(${value.map((v) => exports.parseValue(v)).join(",")})`
  if (exports.isObject(value)) {
    ...
    if (exports.isConstantEscapeId(value, false)) {
      ...
      return escapeId(value.$eId, forbidQualified)
    }
  }
...
function parseLeftRight(left, right, joinSep) {
  ...
  if (Array.isArray(right))
    ...
    if (exports.isConstantExpression(right, false))
      ...
      parts.push(
        `${exports.parseExpression(left)}=` +
          escapeId(right.$eId, forbidQualified)
      )
    } else
      for (const [op, value] of Object.entries(right)) {
        if (op === "$") {
          ...
          parts.push(
            `between ${exports.parseValue(left)} and ${exports.parseValue(
              value
            )}`
          )
        } else if (op === "$nbtw") {
          parts.push(
            `not between ${exports.parseValue(left)} and ${exports.parseValue(
              value
            )}`
          )
        } else if (op === "$in") {
  ...
  return parts.join(parts.join(joinSep))
}

exports.parseSearchCondition = (condition, joinSep = " and ") => {
  if (Array.isArray(condition)) {
    return `(${condition
      .map((cond) => exports.parseSearchCondition(cond))
      .join(joinSep)})`
  }
...
exports.parseTableReferences = (...refs) => {
  let parsed = ""
  const coma = () => {
    ...
    if (Array.isArray(ref)) {
      ...
      parsed += ` ${ref.prefix || "inner"} join ${exports.parseExpression(
        ...
        ref.on
          ...
          : ref.using
          ? ` using (${ref.using.map((c) => escapeId(c)).join(",")})`
          : ""
      }`
    } else {
      coma()
